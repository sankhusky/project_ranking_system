...
public class TeamComparator {
    public static int compare(Team t1, Team t2, int roundStyle) {
        ...
        if (pd1.containsKey(t2.getName()) && pd1.get(t2.getName()).getCount() > 4) { //// they have record with each other & played more than 3 matches
            return roundOff(pd1.get(t2.getName()).getMean(), roundStyle);
            ...
            return compareByMeanSD(t1.getAvgMean(lst1), t1.getAvgSD(lst1), t2.getAvgMean(lst2), t2.getAvgSD(lst2), roundStyle);
        } else { //general comparison
            return compareByMeanSD(t1.getAvgMean(), t1.getAvgSD(), t2.getAvgMean(), t2.getAvgSD(), roundStyle);
        }
    }
     ...
     */
    public static int compareByMeanSD(double avgMean1, double avgSD1, double avgMean2, double avgSD2, int roundStyle) {
        //compare using means
        ...
        if (difference > 0.3) { //0.3 assumed as a threshold
            return roundOff(avgMean1 - avgMean2, roundStyle ); //high mean high chances to win
        } else { //curves are very similar, need to consider SD for comparison
            return avgSD1 < avgSD2 ? roundOff(avgMean1, roundStyle) : roundOff(avgMean2,roundStyle );  //small sd high
            // accuracy to win
        }
    }

    public static int roundOff(double val, int roundStyle) {
        if(roundStyle==0) {
            if (val <= 0) {
        ...
        }else if (roundStyle==1){
            return (int)Math.round(val);
        }else{ // unlikely to reach here
            return (int)Math.round(val);
        }
    }
}
